...
void class_cube::input() 
{
  ...
  if(full_matrix==1){
    ...
    for(unsigned int i=0;i<nsurx;i++)
      mAx[i]  = (double*)malloc(sizeof(double)*(nsurx-i));
    for(unsigned int i=0;i<nsury;i++)
      mAy[i]  = (double*)malloc(sizeof(double)*(nsury-i));
    for(unsigned int i=0;i<nsurz;i++)
      mAz[i]  = (double*)malloc(sizeof(double)*(nsurz-i));

    for(unsigned int i=0;i<nc;i++)
      {
	mHyxz[i] = (double*)malloc(sizeof(double)*nsurx);
  ...
  if(Btrape==1)
    {
      ...
      for(unsigned int i=0;i<nc_plane;i++){		
  ...
  if(rel==3)
    {
      data = (double***)malloc(sizeof(double**)*measured_fields);

      for(unsigned int i=0;i<measured_fields;i++)
	data[i] = (double**)malloc(sizeof(double*)*measured_points);

      for(i=0;i<measured_fields;i++)
	for(j=0;j<measured_points;j++)
	  data[i][j]	= (double*)malloc(sizeof(double)*4);
    }//[B][angle][0-point, 1-B, 2-Jc, 3-n]

  if(rel==3)
    {
      data1 = (double**)malloc(sizeof(double*)*measured_points);
      for(i=0;i<measured_points;i++)
	data1[i] = (double*)malloc(sizeof(double)*4);
    }//[point][0-point, 1-B, 2-Jc, 3-n]
  ...
  if((Jol==0)&&(rho!=0))
    {
      Jol=(Ec/rho);
    } //current density of linear material
  ...
  B_a[0] = Ba * sin(fi*pi/180.0)*cos(theta*pi/180.0);
  if(fabs(B_a[0])<1e-12) B_a[0]=0; //instant magnetic field B_a x component		

  B_a[1] = Ba * sin(fi*pi/180.0)*sin(theta*pi/180.0);
  if(fabs(B_a[1])<1e-12) B_a[1]=0; //instant magnetic field B_a y component
  
  B_a[2] = Ba * cos(fi*pi/180.0);
  if(fabs(B_a[2])<1e-12) B_a[2]=0; //instant magnetic field B_a z component
...
void class_cube::surfaceX_size(int i, int j, int k, double SS[4])
{
  ...
  if (i==0)
    SS[0] = xnode_rc(i+1,j,k,0)  - xnode_rc(i,j,k,0);
  if ((i!=0)&&(i!=ncx))
    SS[0] = (1.0/2.0)*(xnode_rc(i+1,j,k,0)  - xnode_rc(i,j,k,0)) + (1.0/2.0)*(xnode_rc(i,j,k,0) - xnode_rc(i-1,j,k,0));//a
  if (i==ncx)
    SS[0] = xnode_rc(i,j,k,0) 	 - xnode_rc(i-1,j,k,0);
...
{	
  ...
  if (j==0)
    SS[1] = xnode_rc(i,j+1,k,1) - xnode_rc(i,j,k,1); //b
  if ((j!=0)&&(j!=ncy))
    SS[1] = (1.0/2.0)*(xnode_rc(i,j+1,k,1) - xnode_rc(i,j,k,1)) + (1.0/2.0)*(xnode_rc(i,j,k,1) - xnode_rc(i,j-1,k,1));
  if (j==ncy)
    SS[1] = xnode_rc(i,j,k,1) - xnode_rc(i,j-1,k,1);
}
...
{	
  ...
  if (k==0)
    {SS[2] = xnode_rc(i,j,k+1,2) - xnode_rc(i,j,k,2);}
  if ((k!=0)&&(k!=ncz))
    {SS[2] = (1.0/2.0)*(xnode_rc(i,j,k+1,2) - xnode_rc(i,j,k,2)) + (1.0/2.0)*(xnode_rc(i,j,k,2) - xnode_rc(i,j,k-1,2));} //c
  if (k==ncz)
    {SS[2] = xnode_rc(i,j,k,2) - xnode_rc(i,j,k-1,2);} 
}
...
double class_cube::vol_infX(int i, int j, int k)
{
  ...
  if (i==0)
    {return vc[m].V;} 
  if ((i!=0)&&(i!=ncx))
    {return (1.0/2.0)*vc[m].V + (1.0/2.0)*vc[m-1].V;}
  if (i==ncx)
    {return vc[m-1].V;}
}
...
double class_cube::vol_infY(int i, int j, int k)
{
  ...
  if (j==0)
    {return vc[m].V;}
  if ((j!=0)&&(j!=ncy))
    {return (1.0/2.0)*vc[m].V + (1.0/2.0)*vc[m - ncx].V;}
  if (j==ncy)
    {return vc[m - ncx].V;}
}
...
double class_cube::vol_infZ(int i, int j, int k)
{
  ...
  if (k==0)
    {return vc[m].V;}
  if ((k!=0)&&(k!=ncz))
    {return (1.0/2.0)*vc[m].V + (1.0/2.0)*vc[m - ncx*ncy].V;}
  if (k==ncz)
    {return vc[m - ncx*ncy].V;}
}		
...
double class_cube::xnode_rc(int i, int j, int k, int l)
{
  ...
  if (uni==1)
    { //uniform mesh in cartezian coordinate system
      if(l==0)
	{return cx * i;} 											 	
      if(l==1)
	{return cy * j;}
      if(l==2)
	{
	  r = cz * k;
	  if(shape==4)
	    {r = k * (d_tape/nc_tape) - k/(nc_tape+1)*(d_tape/nc_tape) + k/(nc_tape+1)*d_gap;}
	  return r;
	}
    }
  ...
  if (uni==2){//linear material non-uniform mesh
	...
	{						
	  if (i<=nscx/2)
	    {r = ((x-xl)/nscx)*i;}
	  if ((i>nscx/2)&&(i<=(nscx/2) + nncx))
	    {r = ((x-xl)/2) + (i-(nscx/2))*(xl/nncx);}
	  if (i>(nscx/2) + nncx)
	    {
	      r = ((x-xl)/2) + xl + (i - (nscx/2 + nncx))*((x-xl)/nscx);
	      if (i==ncx) {r=x;}
	    }
	}
      return r;
...
double class_cube::xnode_prc(int i, int j, int k, int l)
{
  if(l==0)
	...
	{return rcx_plane - x_plane/2.0 + pcx[0] * i;}
    }					
  if(l==1)
    {
      if(ncy_plane==1)
	...
	{return rcy_plane - y_plane/2.0 + pcx[1] * j;}
    }
  if(l==2)
    {
      return rcz_plane - pcx[2]/2.0 + pcx[2] * k;
    }
}
  ...
  if(set==2)
      ...
      for(m=0;m<xsall[set];m++)
	{
	  ...
	  if(ncz==1){cik[m]=0;cak[m]=0;}
	    ...
	    if(k==0){cik[m] = 0;}
	    else{cik[m] = nscz*k - (nscz-shiftz2);}
	    if(k==0){cak[m] = shiftz2-1;}
	    else{cak[m] = cik[m] + nscz - 1;}
